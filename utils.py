...
def show_epoch_info(phase, log_path, info):
    ...
    print_log(log_path, 'Group Activity Accuracy_w: %.2f%%,Group Activity Accuracy_wo: %.2f%%, Action Accuracy: %.2f%%, activity_Loss: %.5f,action_Loss: %.5f,membership_Loss: %.5f, Using %.1f seconds'%(
                info['activities_acc_w'],info['activities_acc_wo'], info['actions_acc'],info['activity_loss'], info['action_loss'],info['membership_loss'], info['time']))
